...
9. **Capacité du gabarit — Cadre 9** : le modèle n'offre que **2 blocs = 4 emplacements serveurs**. `fill_da.py` écrit les 4 serveurs cœur (EGAPRO.APP, EGAPRO.NOTIF, EGAPRO.BDD [2 bases : `egapro` + `egapro_notifications`], EGAPRO.CACHE) ; **EGAPRO.APISIX (Apache APISIX 3.11.0) et EGAPRO.AV (ClamAV 1.5.2) ont été ajoutés** dans un 3ᵉ bloc via `postprocess_da.py`. Les cellules Type/Rôle/VCPU/RAM (fusionnées) restent à saisir à la main pour tous les blocs.
10. **Capacité du gabarit — Cadre 12** : le modèle offre **5 blocs URL** ; `fill_da.py` écrit les 5 premières et **les 3 URLs sortantes restantes (SUIT, GIP-MDS, S3 OVH) ont été ajoutées** via `postprocess_da.py`. Les 8 URLs sont présentes et cohérentes avec la matrice des flux (cadre 10).
