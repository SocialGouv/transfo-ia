...
| [03-portage-jira.md](03-portage-jira.md) | Porter la pipeline vers Jira via MCP — **portage réalisé** : architecture confirmée, pièges vérifiés en réel, installation de la version portée. |
| `ref/` | La config issue du portage Jira réel (kit édition Jira). Arbre de travail : `.claude/` ; le projet complet (scripts, `INSTALL-JIRA.md`) est dans son historique git. Le dossier `.agents/` qui s'y trouve appartient à un autre système (usine MAIster-Mind / OpenCode), hors périmètre de cette pipeline. |
