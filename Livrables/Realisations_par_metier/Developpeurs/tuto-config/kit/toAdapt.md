...
Ce kit est câblé GitHub (issues + Project v2 + `gh`). Le portage Jira complet (mapping des opérations vers le MCP Atlassian Rovo / REST, contrainte « MCP ≠ bash ») est dans **`portage-jira.md`** (joint à ce kit). Deux deltas à connaître dès maintenant :
