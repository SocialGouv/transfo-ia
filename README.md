...
<picture>
  ...
  <img alt="Comparaison de 7 stacks sur trois mesures de performance : DeepSWE v1.1 en principal (capacité agentique du modèle seul, harness fixé mini-swe-agent, leaderboard officiel Datacurve), Terminal-Bench 2.1 en agentique, et SWE-bench Verified conservé en baseline grisée (saturé, chiffres surtout éditeurs, harness hétérogènes). DeepSWE : Claude Code avec Opus 5 via Bedrock à 74 % (plus ou moins 4), Fable 5 à 70 %, OpenCode avec Kimi K3 via OpenRouter à environ 69 %, Sonnet 5 non publié, DeepSeek V4 Flash 0731 via Scaleway à 54,4 % (auto-rapporté), GLM 5.2 via Scaleway à 44 %, DeepSeek V4 Flash preview via Albert à 7,3 % auto-rapporté (environ 8 % mesuré en indépendant, checkpoint à confirmer auprès de la DINUM). Terminal-Bench : Opus 5 89,1 %, Kimi K3 88,3 % auto-rapporté, Sonnet 5 85,2 %, DeepSeek 0731 82,7 % auto-rapporté, GLM 81 % auto-rapporté, DeepSeek preview 61,8 % auto-rapporté, non publié pour Fable 5. SWE-bench Verified en baseline : Opus 5 97 % (mesure vals.ai, harness minimal), Fable 5 95 %, Kimi K3 93,4 % auto-rapporté, Sonnet 5 82,1 %, DeepSeek preview 79 %, GLM environ 78 %, non publié pour DeepSeek 0731. Prix entrée / sortie par million de tokens : gratuit via Albert ; 0,40 et 0,80 € pour DeepSeek 0731 via Scaleway ; 1,80 et 5,50 € pour GLM via Scaleway ; 2 et 10 $ pour Sonnet 5 ; 2,55 et 12,75 $ pour Kimi K3 ; 5 et 25 $ pour Opus 5 ; 10 et 50 $ pour Fable 5. Souveraineté : Albert (SecNumCloud, État français) et Scaleway (cloud français) tiennent ; Bedrock est non souverain (CLOUD Act). Conformité RGPD bonne via Bedrock (région UE), Scaleway et Albert ; insuffisante via OpenRouter" src="Etat-avancement/assets/06-bench-light.svg" width="100%">
</picture>
...
- **La ligne DeepSeek est dédoublée** : Scaleway sert le checkpoint **0731** (DeepSWE 54,4 % auto-rapporté), Albert sert a priori la **preview d'avril** (DeepSWE 7,3 % auto-rapporté, ≈8 % mesuré en indépendant — checkpoint à confirmer auprès de la DINUM). Même nom, mais des performances agentiques sans rapport.
...
- **Conformité RGPD** : Bedrock en région UE (DPA AWS) reste une voie valable pour les modèles Anthropic — **Opus 5** (DeepSWE 74 %, tête du bench, à moitié prix de Fable), **Fable 5** (70 %) et **Sonnet 5** (l'entrée de gamme, DeepSWE non publié) ; Scaleway et Albert conformes ; **OpenRouter sans garanties** — à réserver éventuellement aux externes.
